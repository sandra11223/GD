...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 
                          error.message || 
                          'Login failed. Please check if the backend server is running on http://localhost:5000';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0a0f1a] pt-16 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 circuit-pattern opacity-10"></div>
      
      {/* Glowing Dots */}
      <div className="absolute top-1/4 left-1/4 glow-dot animate-pulse"></div>
      <div className="absolute top-2/3 right-1/4 glow-dot animate-pulse delay-300"></div>
      <div className="absolute bottom-1/4 left-1/3 glow-dot animate-pulse delay-600"></div>
